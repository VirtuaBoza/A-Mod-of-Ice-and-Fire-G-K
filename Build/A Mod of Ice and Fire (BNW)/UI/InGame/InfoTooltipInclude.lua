...
function GetHelpTextForUnit(iUnitID, bIncludeRequirementsInfo)
	...
	-- print("GetHelpTextForUnit(" .. pUnitInfo.Type .. ")");
	
	local pActivePlayer = (Game ~= nil) and Players[Game.GetActivePlayer()] or nil
	-- local pActiveTeam = Teams[Game.GetActiveTeam()];

	local strHelpText = "";

	local function getUnitProductionNeeded(iUnitID)
          if (pActivePlayer ~= nil) then
            return pActivePlayer:GetUnitProductionNeeded(iUnitID)
          else
            return pUnitInfo.Cost
          end
	end

	local function getNumResourceRequiredForUnit(iUnitID, iResourceID)
          if (Game ~= nil) then
            return Game.GetNumResourceRequiredForUnit(iUnitID, iResourceID)
          else
            local unitType = GameInfo.Units[iUnitID].Type
            local resourceType = GameInfo.Resources[iResourceID].Type

            local resource = GameInfo.Unit_ResourceQuantityRequirements{UnitType=unitType, ResourceType=resourceType}
	...
	strHelpText = strHelpText .. Locale.ConvertTextKey("TXT_KEY_PRODUCTION_COST", getUnitProductionNeeded(iUnitID));
	...
	for pResource in GameInfo.Resources() do
		...
		iNumResourceNeeded = getNumResourceRequiredForUnit(iUnitID, iResourceID);
...
function GetHelpTextForBuilding(iBuildingID, bExcludeName, bExcludeHeader, bNoMaintenance, pCity)
	...
	-- print("GetHelpTextForBuilding(" .. pBuildingInfo.Type .. ")");
	...
	local function getBuildingYieldChange(iBuildingID, iYieldID)
          if (Game ~= nil) then
            return Game.GetBuildingYieldChange(iBuildingID, iYieldID)
          else
            local buildingType = GameInfo.Buildings[iBuildingID].Type
            local yieldType = GameInfo.Yields[iYieldID].Type

            local yield = GameInfo.Building_YieldChanges{BuildingType=buildingType, YieldType=yieldType}
            if (yield ~= nil) then
              for y in yield do
                return y.Yield
              end
	...
	local function getBuildingYieldModifier(iBuildingID, iYieldID)
          if (Game ~= nil) then
            return Game.GetBuildingYieldModifier(iBuildingID, iYieldID)
          else
            local buildingType = GameInfo.Buildings[iBuildingID].Type
            local yieldType = GameInfo.Yields[iYieldID].Type

            local yield = GameInfo.Building_YieldModifiers{BuildingType=buildingType, YieldType=yieldType}
            if (yield ~= nil) then
              for y in yield do
                return y.Yield
              end
	...
	local function getBuildingProductionNeeded(iBuildingID)
          if (pActivePlayer ~= nil) then
            return pActivePlayer:GetBuildingProductionNeeded(iBuildingID)
          else
            return GameInfo.Buildings[iBuildingID].Cost
          end
        end

	local function getExtraBuildingHappinessFromPolicies(iBuildingID)
          if (pActivePlayer ~= nil) then
            return pActivePlayer:GetExtraBuildingHappinessFromPolicies(iBuildingID)
          else
            return 0
          end
        end

	local function getPolicyBuildingClassYieldModifier(iBuildingID, iYieldID)
          if (pActivePlayer ~= nil) then
            return pActivePlayer:GetPolicyBuildingClassYieldModifier(iBuildingID, iYieldID)
          else
            return 0
          end
        end
	...
	if (not bExcludeHeader) then
		...
		if(pBuildingInfo.Cost > 0) then
			local iCost = getBuildingProductionNeeded(iBuildingID);
	...
	iHappinessTotal = iHappinessTotal + getExtraBuildingHappinessFromPolicies(iBuildingID);
	if (pCity ~= nil) then
	...
	-- Culture
	local iYieldCulture = GameInfo.Yields["YIELD_CULTURE"].ID
	local iCulture = getBuildingYieldChange(iBuildingID, iYieldCulture);
	if (pCity ~= nil) then
		iCulture = iCulture + pCity:GetReligionBuildingClassYieldChange(buildingClassID, iYieldCulture) + pActivePlayer:GetPlayerBuildingClassYieldChange(buildingClassID, iYieldCulture);
	end
	...
	-- Faith
	local iYieldFaith = GameInfo.Yields["YIELD_FAITH"].ID
	local iFaith = getBuildingYieldChange(iBuildingID, iYieldFaith);
	if (pCity ~= nil) then
		iFaith = iFaith + pCity:GetReligionBuildingClassYieldChange(buildingClassID, iYieldFaith) + pActivePlayer:GetPlayerBuildingClassYieldChange(buildingClassID, iYieldFaith);
	end
	...
	-- Food
	local iYieldFood = GameInfo.Yields["YIELD_FOOD"].ID
	local iFood = getBuildingYieldChange(iBuildingID, iYieldFood);
	if (pCity ~= nil) then
		iFood = iFood + pCity:GetReligionBuildingClassYieldChange(buildingClassID, iYieldFood) + pActivePlayer:GetPlayerBuildingClassYieldChange(buildingClassID, iYieldFood);
	end
	...
	-- Gold Mod
	local iYieldGold = GameInfo.Yields["YIELD_GOLD"].ID
	local iGold = getBuildingYieldModifier(iBuildingID, iYieldGold);
	iGold = iGold + getPolicyBuildingClassYieldModifier(buildingClassID, iYieldGold);
	...
	-- Gold Change
	iGold = getBuildingYieldChange(iBuildingID, iYieldGold);
	if (pCity ~= nil) then
		iGold = iGold + pCity:GetReligionBuildingClassYieldChange(buildingClassID, iYieldGold) + pActivePlayer:GetPlayerBuildingClassYieldChange(buildingClassID, iYieldGold);
	end
	if (iGold ~= nil and iGold ~= 0) then
	...
	-- Science
	local iYieldScience = GameInfo.Yields["YIELD_SCIENCE"].ID
	local iScience = getBuildingYieldModifier(iBuildingID, iYieldScience);
	iScience = iScience + getPolicyBuildingClassYieldModifier(buildingClassID, iYieldScience);
	if (iScience ~= nil and iScience ~= 0) then
		table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_SCIENCE", iScience));
	end
	
	-- Science
	local iScienceChange = getBuildingYieldChange(iBuildingID, iYieldScience);
	if (pCity ~= nil) then
		iScienceChange = iScienceChange + pCity:GetReligionBuildingClassYieldChange(buildingClassID, iYieldScience) + pActivePlayer:GetPlayerBuildingClassYieldChange(buildingClassID, iYieldScience);
	end
	...
	-- Production
	local iYieldProduction = GameInfo.Yields["YIELD_PRODUCTION"].ID
	local iProduction = getBuildingYieldModifier(iBuildingID, iYieldProduction);
	iProduction = iProduction + getPolicyBuildingClassYieldModifier(buildingClassID, iYieldProduction);
	...
	-- Production Change
	local iProd = getBuildingYieldChange(iBuildingID, iYieldProduction);
	if (pCity ~= nil) then
		iProd = iProd + pCity:GetReligionBuildingClassYieldChange(buildingClassID, iYieldProduction) + pActivePlayer:GetPlayerBuildingClassYieldChange(buildingClassID, iYieldProduction);
	end
...
function GetHelpTextForImprovement(iImprovementID, bExcludeName, bExcludeHeader, bNoMaintenance)
	...
	-- print("GetHelpTextForImprovement(" .. pImprovementInfo.Type .. ")");
